  ...
  IHttpContextAccessor httpContextAccessor) : IAuthService
{
  public async Task<AuthResponseDto> LoginAsync(LoginRequestDto request)
  {
    ...
    command.CommandText = """
      SELECT vpu.id, vpu.vendor_id, vpu.email, vpu.pass_hash, vpu.full_name, vpu.status, v.status AS vendor_status
    ...
    if (!await reader.ReadAsync() ||
        ...
        reader.GetString(reader.GetOrdinal("vendor_status")) != "APPROVED" ||
        !BCrypt.Net.BCrypt.Verify(request.Password, reader.GetString(reader.GetOrdinal("pass_hash"))))
      throw new UnauthorizedAccessException("auth.invalid_credentials");

    var id = reader.UInt64("id");
    var vendorId = reader.UInt64("vendor_id");
  ...
  public async Task<UserResponseDto> GetCurrentUserAsync()
  {
    ...
    var id = ulong.Parse(principal.FindFirstValue(ClaimTypes.NameIdentifier) ?? throw new UnauthorizedAccessException());
  ...
  public async Task<AuthResponseDto> RefreshAsync(string refreshToken)
  {
    ...
    var vendorIdText = principal.FindFirstValue("vendor_id");
    ulong? vendorId = string.IsNullOrWhiteSpace(vendorIdText) ? null : ulong.Parse(vendorIdText);
    ...
    if (role == "VENDOR")
    {
      ...
      await using var command = connection.CreateCommand();
      command.CommandText = """
        SELECT vpu.status,v.status FROM vendor_portal_users vpu
      ...
      await using var reader = await command.ExecuteReaderAsync();
      if (!await reader.ReadAsync() || reader.GetString(0) != "ACTIVE" || reader.GetString(1) != "APPROVED")
  ...
  private AuthResponseDto CreateToken(UserResponseDto user, ulong? vendorId)
  {
    ...
    if (vendorId.HasValue) claims.Add(new("vendor_id", vendorId.Value.ToString()));
