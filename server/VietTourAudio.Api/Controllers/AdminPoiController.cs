  ...
  VietTourAudio.Api.Services.PoiTranslationService translationService) : ControllerBase
{
  ...
  public async Task<IActionResult> Create([FromBody] JsonElement body)
  {
    ...
    var tourId = body.GetProperty("tourId").GetString() ?? "";
    var stallId = body.GetProperty("stallId").GetString() ?? "";
    ...
    if (await db.Pois.AsNoTracking().AnyAsync(poi => poi.VendorId == stallId && poi.Slug == slug))
      return Conflict(ApiResponseFactory.Fail("poi.slug_exists"));

    var coverUrl = body.TryGetProperty("coverUrl", out var coverUrlVal) && coverUrlVal.ValueKind != JsonValueKind.Null ? coverUrlVal.GetString() : null;
    var entity = new Poi
    {
  ...
  public async Task<IActionResult> Update(string id, [FromBody] JsonElement body)
  {
    ...
    var newCoverUrl = body.TryGetProperty("coverUrl", out var coverUrlVal) && coverUrlVal.ValueKind != JsonValueKind.Null ? coverUrlVal.GetString() : null;

    poi.FestivalZoneId = body.GetProperty("tourId").GetString() ?? "";
    poi.VendorId = body.GetProperty("stallId").GetString();
    poi.StallName = name;
    poi.Slug = body.TryGetProperty("slug", out var slugValue) ? slugValue.GetString() ?? Slugify(name) : Slugify(name);
    poi.Description = description;
    poi.Latitude = (double)body.GetProperty("latitude").GetDecimal();
    poi.Longitude = (double)body.GetProperty("longitude").GetDecimal();
    poi.TriggerRadius = body.GetProperty("activationRadius").GetInt32();
    poi.IsPremiumPriority = body.GetProperty("isPremiumContent").GetBoolean();
    poi.Status = body.GetProperty("status").GetString() ?? "ACTIVE";

    if (body.TryGetProperty("coverUrl", out _))
    {
