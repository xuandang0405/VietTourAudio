...
set "API_PORT=5000"
set "API_BASE_URL=https://be.bkpvp.top/api"
...
start "VietTourAudio - .NET API" cmd /k "pushd "%ROOT%server\VietTourAudio.Api" && set ASPNETCORE_ENVIRONMENT=Development && set ASPNETCORE_URLS=http://0.0.0.0:5000 && set ConnectionStrings__DefaultConnection=%DOTNET_CONNECTION% && set Storage__PublicBaseUrl=https://be.bkpvp.top/uploads && dotnet run --no-launch-profile"
echo   [OK]  Unified API : https://be.bkpvp.top
echo   [OK]  Swagger     : https://be.bkpvp.top/swagger
