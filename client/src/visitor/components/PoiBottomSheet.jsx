          ...
          >
            ...
            <div className="max-h-[calc(84vh-1rem)] overflow-y-auto px-4 pb-6 pt-4 hide-scrollbar">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600">Đánh giá {poi.rating}/5</span>
              </div>

              <section className="mt-5 rounded-3xl bg-slate-50 p-4">
                <h3 className="font-black text-slate-950">Bản chữ miễn phí</h3>
                <p className="mt-2 max-h-36 overflow-y-auto pr-1 text-sm leading-7 text-slate-600 hide-scrollbar">{poi.description}</p>
              </section>
              ...
              <section className="mt-4 rounded-3xl border border-teal-100 bg-white p-4 shadow-lg shadow-slate-900/5">
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="font-black text-slate-950">{isPremium ? 'Đã mở khóa' : 'Audio đang khóa'}</h3>
                  </div>
                  {isPremium ? <Volume2 className="text-teal-700" size={22} /> : <Lock className="text-slate-400" size={22} />}
                </div>
                ...
                {isPremium ? (
                  <div className="rounded-3xl bg-teal-50 p-4">
                    ...
                    <p className="mt-3 text-center text-xs font-semibold text-teal-800/70">Ngôn ngữ đang phát: {currentLanguage.toUpperCase()}</p>
                  </div>
                ) : (
                  <div className="relative overflow-hidden rounded-3xl bg-slate-100 p-4">
                    ...
                    <div data-testid="audio-locked-state" className="absolute inset-0 grid place-items-center bg-white/60 px-5 text-center backdrop-blur-[2px]">
                      <div>
                        ...
                        <p className="mt-2 text-sm font-black text-slate-800">Chỉ bản chữ. Quét QR để mở khóa Giọng đọc AI</p>
                      </div>
                    </div>
                    <button
                      type="button"
