...
import { Lock, RefreshCw, Volume2, X } from 'lucide-react';
...
export function PoiBottomSheet({ poi, onClose, onUpgrade, onToast }) {
  ...
  const lastError = useAudioStore((state) => state.lastError);
  ...
  function handleReplay() {
    ...
    onToast?.(played ? 'Đang phát lại audio thuyết minh.' : lastError || 'Không thể phát audio trên trình duyệt này.');
  }
          ...
          >
            ...
            <div className="max-h-[calc(84vh-1rem)] overflow-y-auto px-4 pb-6 pt-4 hide-scrollbar">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <span className="rounded-full border border-glassBorder bg-white/5 px-3 py-1 text-xs font-bold text-textSeafoam">Đánh giá {poi.rating}/5</span>
              </div>

              <section className="mt-5 rounded-2xl border border-glassBorder bg-white/5 p-4 shadow-glass-inner">
                <h3 className="font-display font-bold text-textCrisp">Bản chữ miễn phí</h3>
                <p className="mt-2 max-h-36 overflow-y-auto pr-1 text-sm leading-7 text-textSeafoam hide-scrollbar">
                  {poi.description || 'Nội dung thuyết minh đang được cập nhật.'}
                </p>
              </section>
              ...
              <section className="mt-4 rounded-2xl border border-glassBorder bg-glassCore p-4 shadow-glass-inner">
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="font-display font-bold text-textCrisp">{isPremium ? 'Đã mở khóa' : 'Audio đang khóa'}</h3>
                  </div>
                  {isPremium ? <Volume2 className="text-oceanCyan" size={22} /> : <Lock className="text-textGhost" size={22} />}
                </div>
                ...
                {isPremium ? (
                  <div className="rounded-2xl border border-oceanCyan/20 bg-oceanCyan/10 p-4">
                    ...
                    <p className="mt-3 text-center text-xs font-semibold text-textSeafoam">Ngôn ngữ đang phát: {currentLanguage.toUpperCase()}</p>
                  </div>
                ) : (
                  <div className="relative overflow-hidden rounded-2xl border border-glassBorder bg-bgElevated/50 p-4">
                    ...
                    <div data-testid="audio-locked-state" className="absolute inset-0 grid place-items-center bg-bgAbyss/62 px-5 text-center backdrop-blur-[2px]">
                      <div>
                        ...
                        <p className="mt-2 text-sm font-bold text-textCrisp">Chỉ bản chữ. Quét QR để mở khóa Giọng đọc AI</p>
                      </div>
                    </div>
                    <button
                      type="button"
