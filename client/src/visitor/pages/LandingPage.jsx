  ...
  return (
    <section className="relative flex h-[100vh] min-h-[100vh] w-full overflow-hidden bg-transparent font-body text-textCrisp">
      ...
      <div className="relative z-10 flex h-full w-full flex-shrink-0 flex-col px-6 pb-12 pt-32 pc:basis-[45%] pc:border-r pc:border-glassBorder pc:bg-bgAbyss/80 pc:px-10 xl:px-14">
        ...
        <div className="mx-auto mt-auto flex w-full max-w-md flex-1 flex-col justify-center pc:mx-0 pc:mt-16 pc:max-w-none">
          <div className="mb-8 flex flex-col items-center gap-4 pc:mb-12 pc:items-start">
            ...
            <span className="font-display text-4xl font-bold leading-none text-transparent bg-gradient-to-r from-textCrisp via-electricBlue to-oceanCyan bg-clip-text drop-shadow-sm tablet:text-5xl">
              VietTourAudio
            </span>
          </div>
          ...
          <h1 className="mb-4 text-center font-display text-4xl font-bold leading-[1.1] text-textCrisp pc:mb-6 pc:text-left pc:text-5xl xl:text-6xl">
            Khám phá vẻ đẹp qua <span className="bg-gradient-to-r from-electricBlue to-oceanCyan bg-clip-text text-transparent">giọng kể AI</span>
          </h1>
              ...
              {languages.map((language) => {
                ...
                return (
                  <button
                    ...
                    aria-pressed={active}
                    className={[
                      'rounded-xl border px-3 py-2 text-sm font-bold transition duration-150 ease-out active:scale-[0.98]',
                      active
          ...
          <div className="mx-auto grid w-full max-w-sm gap-4 pc:mx-0 pc:max-w-none pc:flex pc:flex-col xl:flex-row">
            ...
            >
              ...
              {permissionStatus === 'requesting' ? 'Đang xin quyền...' : 'Bắt đầu ngay'}
            </button>
          ...
          {permissionStatus === 'denied' && (
            <div className="mt-6 rounded-2xl border border-error/25 bg-error/10 p-4">
              <p className="text-center text-sm font-medium leading-relaxed text-red-100 pc:text-left">
          ...
          <div className="relative z-10 mx-auto mt-10 grid w-full max-w-sm grid-cols-2 gap-3 border-t border-white/10 pt-8 pc:mx-0 pc:mt-auto pc:max-w-none">
            ...
            <FeatureBadge icon={Headphones} label={isPremium ? 'Audio Premium' : 'Text và ảnh'} isPremium={isPremium} />
          </div>
        </div>
      </div>
      ...
      <div className="relative hidden flex-1 bg-bgTrench pc:block">
        ...
        <img src={heroTravel} alt="Du lịch Việt Nam" className="absolute inset-0 h-full w-full object-cover opacity-80" loading="lazy" decoding="async" />
        <div className="absolute inset-x-0 bottom-0 z-20 p-16">
          <div className="max-w-xl">
            <h3 className="mb-2 font-display text-2xl font-bold text-textCrisp">Trải nghiệm âm thanh sống động</h3>
            <p className="leading-relaxed text-textSeafoam">
              Ứng dụng tự động nhận diện vị trí của bạn và phát thuyết minh chuyên sâu về lịch sử, kiến trúc và văn hóa của từng địa điểm.
            </p>
          </div>
        </div>
      </div>
...
function FeatureBadge({ icon: Icon, label, isPremium }) {
  return (
    <div className={isPremium ? 'glass-card glass-card-active flex flex-col items-center justify-center p-4 pc:items-start' : 'glass-card flex flex-col items-center justify-center p-4 pc:items-start'}>
      ...
      <span className={isPremium ? 'mt-2 text-xs font-bold uppercase text-premiumNeon' : 'mt-2 text-xs font-bold uppercase text-textSeafoam'}>
        {label}
      </span>
    </div>
  );
