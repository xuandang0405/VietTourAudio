...
import { axiosClient as adminApiClient } from '../../api/axiosClient';
...
adminApiClient.interceptors.response.use(
  ...
  async (error) => {
    if (error.response) {
      console.error(
        `[Admin API Error] ${error.config?.method?.toUpperCase()} ${error.config?.url} failed with status ${error.response.status}:`,
        error.response.data
      );
    } else {
      console.error(`[Admin API Error] Network or setup error:`, error.message);
    }
...
export async function adminLogin(credentials) {
  const url = `${import.meta.env.VITE_API_BASE_URL || appConfig.adminApiBaseUrl}/admin/auth/login`;
  return unwrap(await adminApiClient.post(url, credentials));
}
