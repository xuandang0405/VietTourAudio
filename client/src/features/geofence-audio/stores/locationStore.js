...
export const useLocationStore = create((set, get) => ({
  ...
  updateAccuracy: (isHighAccuracy) => {
    const current = get().isHighAccuracy;
    if (current === isHighAccuracy) return;
    set({ isHighAccuracy });
    const { watchId } = get();
    if (watchId) {
      get().stopWatching();
  ...
  requestLocation: () => {
    ...
    return new Promise((resolve) => {
      ...
      navigator.geolocation.getCurrentPosition(
        successCallback,
        errorCallback,
        { enableHighAccuracy: get().isHighAccuracy, timeout: 15000, maximumAge: 15000 }
      );
    });
  },
  ...
  startWatching: () => {
    ...
    const id = navigator.geolocation.watchPosition(
      handleSuccess,
      handleError,
      { enableHighAccuracy: get().isHighAccuracy, timeout: 15000, maximumAge: 0 }
    );
