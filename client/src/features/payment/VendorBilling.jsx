...
export function VendorBilling() {
  ...
  function activateVendorEntitlement(transactionType) {
    const premiumExpiryDate = new Date();
    premiumExpiryDate.setMonth(premiumExpiryDate.getMonth() + 1);
    setSession({
      user: {
        ...user,
        isPremium: transactionType === 'VENDOR_PREMIUM' ? true : user?.isPremium,
        premiumExpiryDate: transactionType === 'VENDOR_PREMIUM'
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <CheckoutMatrix
        ...
        onSuccess={() => activateVendorEntitlement(type)}
        onSuccessClose={() => navigate('/vendor/stall')}
      />
    </div>
  );
}
