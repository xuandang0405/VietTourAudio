  ...
  async function handleConfirm() {
    ...
    try {
      if (modal.type === 'delete') {
        ...
        formData.append('tourId', formTourId);
        formData.append('stallId', formStallId);
        formData.append('latitude', String(lat));
        formData.append('longitude', String(lng));
        formData.append('activationRadius', String(rad));
        formData.append('isPremiumContent', String(formIsPremium));
        formData.append('status', formStatus);
        formData.append('description', formDescription.trim());
        if (formCoverFile) {
          formData.append('coverFile', formCoverFile);
        } else if (formCoverImageUrl) {
          formData.append('coverUrl', formCoverImageUrl);
        }
        formData.append('translationsJson', JSON.stringify(formTranslations));

        if (modal.type === 'add') await createMutation.mutateAsync(formData);
        else await updateMutation.mutateAsync({ id: modal.poi.id, data: formData });
      }
      setDeleteReason('');
      setModal(null);
    } catch (err) {
    ...
    <div className="mx-auto max-w-[1600px] space-y-5">
      ...
      >
        <POIForm
          ...
          tours={tours}
          error={error}
          formCoverFile={formCoverFile}
          setFormCoverFile={setFormCoverFile}
          formPreviewUrl={formPreviewUrl}
          setFormPreviewUrl={setFormPreviewUrl}
