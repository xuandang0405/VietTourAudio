...
export const useFavoritesStore = create(
  persist(
    (set, get) => ({
      favorites: [], // Array of stall IDs (number)
      ...
      syncOfflineOps: async () => {
        ...
        try {
          const response = await apiClient.post('/guest/favorites/sync', {
            guestId,
            ops: queue
          });
          ...
          if (data && Array.isArray(data.favorites)) {
            ...
            set({ favorites: data.favorites.map(Number), localQueue: [] });
          }
        } catch (error) {
      ...
      fetchFavorites: async () => {
        const guestId = getVisitorSessionId();
        try {
          const response = await apiClient.get(`/guest/favorites/${guestId}`);
          const data = response.data?.data ?? response.data;
          if (data && Array.isArray(data.favorites)) {
            // Only update favorites from server if there is no local changes queued
            if (get().localQueue.length === 0) {
              set({ favorites: data.favorites.map(Number) });
            }
          }
        } catch (error) {
