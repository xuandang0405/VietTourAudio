...
export const useLocationStore = create((set, get) => ({
    ...
    return new Promise((resolve) => {
      navigator.geolocation.getCurrentPosition(
        (currentPosition) => {
          ...
          get().startWatching(); // Bắt đầu theo dõi sau khi có quyền
          resolve(true);
        },
        (error) => {
          set({
            permissionStatus: 'denied',
            lastError: error.code === 1 
              ? 'Quyền vị trí bị từ chối. Vui lòng cấp quyền trong cài đặt trình duyệt.' 
              : error.code === 2
              ? 'Không thể xác định vị trí hiện tại (Tín hiệu kém).'
              : 'Quá thời gian lấy vị trí. Vui lòng thử lại ở ngoài trời.'
          });
          resolve(false);
        },
        // Tăng timeout cho Mobile: Mobile có thể mất 15-20s để bắt GPS
  ...
  startWatching: () => {
    ...
    if (watchId || isFakeMode) return;

    let lastUpdate = 0;
    const THROTTLE_MS = 5000; // Giới hạn 5s cập nhật 1 lần
    ...
    const id = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        // Không set lastError liên tục ở watchPosition để tránh chớp giật UI,
        // thiết bị có thể rớt GPS tạm thời và có lại ngay.
      },
